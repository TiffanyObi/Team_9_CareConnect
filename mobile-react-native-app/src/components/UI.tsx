...
export function AppText({ children, style, heading = false, secondary = false }: { children: React.ReactNode; style?: StyleProp<TextStyle>; heading?: boolean; secondary?: boolean }): React.JSX.Element {
  ...
  return <Text accessibilityRole={heading ? 'header' : undefined} maxFontSizeMultiplier={2} style={[{ color: secondary ? c.secondaryText : c.text, fontSize: size * settings.textScale, lineHeight: size * 1.5 * settings.textScale }, heading && styles.heading, style, { fontSize: size * settings.textScale, lineHeight: (customStyle.lineHeight ?? size * 1.5) * settings.textScale }]}>{children}</Text>;
}
export function Button({ label, onPress, secondary = false, danger = false, disabled = false, selected = false }: { label: string; onPress?: () => void; secondary?: boolean; danger?: boolean; disabled?: boolean; selected?: boolean }): React.JSX.Element {
  ...
  return <Pressable accessible focusable={!disabled} accessibilityRole="button" accessibilityLabel={label} accessibilityState={{ disabled, selected }} disabled={disabled} onPress={onPress} style={({ pressed }) => [styles.button, { minHeight: settings.largeTouchTargets ? 52 : preferredTouchTarget, minWidth: minimumTouchTarget, backgroundColor, borderColor: secondary && !selected ? c.primary : backgroundColor, opacity: disabled ? .42 : pressed ? .7 : 1 }]}><Text maxFontSizeMultiplier={2} style={{ color: danger ? '#FFFFFF' : selected ? c.onAccent : secondary ? c.primary : c.onPrimary, fontWeight: '700', fontSize: 16 * settings.textScale, lineHeight: 24 * settings.textScale }}>{label}</Text></Pressable>;
}
