...
const symbols = { Today: '⌂', Medications: '▣', Care: '▦', Messages: '✉', Settings: '⚙' };
function Workspace() {
  return <Tabs.Navigator screenOptions={({ route }) => ({
    headerShown: false, animation: 'none', tabBarAccessibilityLabel: route.name + ' tab',
    tabBarIcon: ({ color }) => <Text accessible={false} style={{ color, fontSize: 20 }}>{symbols[route.name]}</Text>,
    tabBarStyle: { minHeight: 64 },
  })}>
    <Tabs.Screen name="Today" component={TodayScreen} />
    <Tabs.Screen name="Medications" component={MedicationsScreen} options={{ title: 'Meds' }} />
    <Tabs.Screen name="Care" component={CareScreen} />
    <Tabs.Screen name="Messages" component={MessagesScreen} />
    <Tabs.Screen name="Settings" component={SettingsScreen} />
  </Tabs.Navigator>;
}
export function RootNavigator() {
  const { signedIn, reducedMotion } = useApp();
  if (!signedIn) return <SignInScreen />;
  return <Stack.Navigator screenOptions={{ animation: reducedMotion ? 'none' : 'default', gestureEnabled: !reducedMotion, headerBackTitle: 'Back' }}>
    <Stack.Screen name="Tabs" component={Workspace} options={{ headerShown: false }} />
